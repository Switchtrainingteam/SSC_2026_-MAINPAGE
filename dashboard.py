...
with col1:
    ...
    st.link_button("Mac", "https://levelssc2026aprilquest-l2vqlcqdsadg8r6bmgqyei.streamlit.app", use_container_width=True)

with col2:
    st.header("🥈")
    st.subheader("Adventurer")
    st.write("Bravery and Skill")
    st.write("Go to Level 2")
    st.link_button("Role-play", "", use_container_width=True)

with col3:
    st.header("🥇")
    st.subheader("Master")
    st.write("Legacy and Ultimate Challenge")
    st.write("Go to Level 3")
    st.link_button("Short Video", "", use_container_width=True)
